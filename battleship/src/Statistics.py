...
class Statistics:
    ...
    def endGame(self):
        ...
        for agentName in self.agentNames:
            position =  self.logBuffer[agentName].tell()
            self.logBuffer[agentName].seek(0, os.SEEK_END)
            size =  self.logBuffer[agentName].tell()
            self.logBuffer[agentName].seek(position, os.SEEK_SET)
            if size > 0:
