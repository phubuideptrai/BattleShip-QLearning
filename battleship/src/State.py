...
class State:
    ...
    def isEnd(self):
        ...
        for agent, torpedoList in self.torpedos.items():
            sumTorpedos = 0
            for (torpedo, count) in torpedoList.items():
                sumTorpedos += count
            if sumTorpedos <= 0:
                winScores= copy.deepcopy(self.scores)
                del winScores[agent]
